            ...
            <div class="row h-100">
                <div class="col-md-6 h-100 d-flex flex-column">
                    <div class="w-100 d-flex border-bottom border-dark py-1 mb-1">
                        <div class="fs-5 col-auto flex-grow-1"><b>Список категорий</b></div>
                    ...
                    <div class="h-100 overflow-auto border rounded-1 border-dark">
                        <ul class="list-group">
                                ...
                                <div class="col-auto pe-2">
                                    <?php 
                                        if(isset($row['status']) && $row['status'] == 1){
                                            echo "<small><span class='badge rounded-pill bg-success'>Активна</span></small>";
                                        }else{
                                            echo "<small><span class='badge rounded-pill bg-danger'>Не активна</span></small>";
                                        }
                                    ?>
                                </div>
                            ...
                            <?php if($cat_qry->num_rows <= 0): ?>
                                <li class="list-group-item text-center">Данные еще не указаны.</li>
                            <?php endif; ?>
                        </ul>
                    </div>
                </div>
...
<script>
    $(function(){
        ...
        $('#new_category').click(function(){
            uni_modal('Добавить новую категорию',"manage_category.php")
        })
        $('.edit_category').click(function(){
            uni_modal('Изменить категорию',"manage_category.php?id="+$(this).attr('data-id'))
        })
        $('.view_category').click(function(){
            uni_modal('Детали категории',"view_category.php?id="+$(this).attr('data-id'))
        })
        $('.delete_category').click(function(){
            _conf("Вы уверены что хотите удалить <b>"+$(this).attr('data-name')+"</b> из списка категорий?",'delete_category',[$(this).attr('data-id')])
        })
    })
