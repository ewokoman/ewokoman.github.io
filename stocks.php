...
<div class="card rounded-0 shadow">
    <div class="card-header d-flex justify-content-between">
        ...
        <div class="card-tools align-middle">
            <button class="btn btn-dark btn-sm py-1 rounded-0" type="button" id="create_new">Добавить новую</button>
        </div>
    </div>
    <div class="card-body">
        <table class="table table-hover table-striped table-bordered">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-center p-0">Дата добавления</th>
                    <th class="text-center p-0">Товар</th>
                    <th class="text-center p-0">Количество</th>
                    <th class="text-center p-0">Срок действия</th>
                ...
                <tr class="<?php echo strtotime(date("Y-m-d")) > strtotime($row['expiry_date']) ? 'bg-danger bg-opacity-50' : '' ?>">
                    ...
                    <td class="text-center py-0 px-1">
                        <div class="btn-group" role="group">
                            ...
                            <li><a class="dropdown-item edit_data" data-id = '<?php echo $row['stock_id'] ?>' href="javascript:void(0)">Изменить</a></li>
                            <li><a class="dropdown-item delete_data" data-id = '<?php echo $row['stock_id'] ?>' data-name = '<?php echo $row['product_code']." - ".$row['pname'] ?>' href="javascript:void(0)">Удалить</a></li>
                            </ul>
                        </div>
                    </td>
                </tr>
...
<script>
    $(function(){
        $('#create_new').click(function(){
            uni_modal('Добавить новую акцию',"manage_stock.php")
        })
        $('.edit_data').click(function(){
            uni_modal('Измениь акцию',"manage_stock.php?id="+$(this).attr('data-id'))
        })
        $('.view_data').click(function(){
            uni_modal('Детали акцию',"view_stock.php?id="+$(this).attr('data-id'),'')
        })
        $('.delete_data').click(function(){
            _conf("Вы уверены что хотите удалить <b>"+$(this).attr('data-name')+"</b> из списка акций?",'delete_data',[$(this).attr('data-id')])
        })
